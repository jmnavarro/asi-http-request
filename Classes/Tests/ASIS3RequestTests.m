...
{	
	...
	int i;
	for (i=0; i<5; i++) {
		NSString *text = [NSString stringWithFormat:@"This is the content of file #%u",i];
		NSString *filePath = [[self filePathForTemporaryTestFiles] stringByAppendingPathComponent:[NSString stringWithFormat:@"%u.txt",i]];
		[[text dataUsingEncoding:NSUTF8StringEncoding] writeToFile:filePath atomically:NO];
		NSString *key = [NSString stringWithFormat:@"test-file/%u",i];
		ASIS3ObjectRequest *request = [ASIS3ObjectRequest PUTRequestForFile:filePath withBucket:bucket key:key];
		[request setSecretAccessKey:secretAccessKey];
		[request setAccessKey:accessKey];
		[request startSynchronous];
	...
	for (ASIS3BucketObject *object in [listRequest objects]) {
		NSString *oldFilePath = [[self filePathForTemporaryTestFiles] stringByAppendingPathComponent:[NSString stringWithFormat:@"%u.txt",i]];;
...
- (void)testQueueProgress
{
	...
	for (i=0; i<5; i++) {
		...
		NSString *key = [NSString stringWithFormat:@"stuff/file%u.txt",i+1];
	...
	for (i=0; i<5; i++) {
		...
		[s3Request setSecretAccessKey:secretAccessKey];
		[s3Request setAccessKey:accessKey];
		NSString *downloadPath = [[self filePathForTemporaryTestFiles] stringByAppendingPathComponent:[NSString stringWithFormat:@"%u.jpg",i+1]];
